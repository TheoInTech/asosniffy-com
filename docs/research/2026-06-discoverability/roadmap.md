...
- **Screenshot captions are OCR-indexed** since ~June 2025 (Appfigures natural experiment) — screenshots are now double-duty: #1 conversion element AND a rank surface. [MECH, needs replication — single source]
- **Deterministic metadata mechanics**: tokens combine across title/subtitle/keyword-field within one locale; plurals stem as duplicates; camelCase splits; cross-field dupes waste bytes; **each territory indexes multiple locales' metadata (US ≈ 10)** — cross-localization is ~10x keyword space for pure metadata work. [MECH, locale table needs verification]
...
**Play search rank** — title(30) > short desc(80) > long desc(4000) indexed; 2-3 natural mentions, stuffing penalized. Uniquely, **the behavioral side is published policy**: crash 1.09%/8% per-device, ANR 0.47%/8% suppress discoverability; "Core Value" gates (DAU/MAU <8%, user-loss >5% → surface ineligibility) [verify exact doc before quoting as policy]. **On Android, engineering quality IS ASO.** AI layer: Ask Play / Gemini discovery reads listings, reviews, and external web content — reviews are now positioning data.
...
1. **No agent-payable ASO data exists anywhere.** AppTweak API floor $166/mo (ships llms.txt but funnel dead-ends at a credit card); Sensor Tower median contract $74k/yr; MobileAction API quote-only; Appfigures MCP needs account+plan; the only per-request alternative (Apify actor, $20/1k keywords) is deprecated. **Sniffy's 402 offer is the only price list an agent can pay with money it holds.** The moat sentence: competitors' marginal cost ≈ Sniffy's price, but their *minimum* price is a seat + a human; following Sniffy means cannibalizing their subscription base.
...
3. **Empty lanes:** nobody audits app-specific web markup (requires store-identity + web-crawl join Sniffy uniquely has); LLM visibility for apps has exactly one player (AppTweak, April 2026, enterprise demo-gated); conversion intelligence is agency/dashboard-locked (SplitMetrics/Storemaven); competitor tracking is brutally quota'd (Appfigures: ONE competitor at $149.99/mo).
...
2.4 **New-surface readiness checklist** (knowledge-grade, `inferred`): ChatGPT App Directory presence/viability + competitor presence; Apple App Intents/IndexedEntity checklist ("is your app legible to Apple Intelligence") — recommendations, not measurements.

### Wave 3 — Deepen the stores (gated items)

3.1 **Cross-localization arithmetic** (gated V3): iOS indexed-but-empty locale slots per target country with fill-ready per-locale keyword sets (extends shipped franc module + multi-storefront provider); Android `hl/gl` machine-translation fingerprint ("auto-translated short desc + English title" = neglected locale).
3.2 **Screenshot caption OCR rank audit** (gated V1): caption extraction (same vision pass as 1.2), keyword alignment vs §1 targets, OCR-illegibility and passive-caption flags. `screenshotIndexing` section.
...
- **Knowledge corpus:** new topics (rating curve/thresholds, OCR captions, Core Value gates, PPO/experiments math, featuring pitch, paid-UA halo, App Intents) added to `packages/aso-knowledge` + scraper mirror; sync test guards drift.
