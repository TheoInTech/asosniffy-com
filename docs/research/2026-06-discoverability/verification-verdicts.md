...
| V5 | LLM probe variance | **BLOCKED — OpenAI quota exhausted (2026-06-10)** | Pilot script is ready (`scraper/scripts/v5-probe-pilot.ts`: 5 apps × 10 prompts × 10 replicates, ~$0.7 on gpt-5.4-mini; run `npx tsx --env-file=.env scripts/v5-probe-pilot.ts`). All 500 calls returned 429 "exceeded your current quota" — the OpenAI account needs billing top-up first. ⚠️ **Side-finding: if Railway uses the same key, production standard/expert synthesis is currently template-fallback** — check the `openai_cost` / fallback telemetry in Railway logs. Multi-model spread (Haiku/Gemini) measured separately at Wave 2.1 build. |
