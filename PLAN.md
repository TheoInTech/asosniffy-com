...
**Wave 1 additions (2026-06, `reportVersion` ≥ `2026-06-mvp-5`; all additive with null/default back-compat):**

- **Request calibration (optional):** `currentKeywordsField` (string ≤100 — the ASC keyword field, not publicly visible; unlocks the full mechanics lint) and `ascDailyImpressions` (number — App Store Connect impressions/day; unlocks the experiment-feasibility verdict). Both paste-in; absence degrades the dependent sections to honest nulls/notes.
- **`shallowScan` teasers (free /quote, §22-safe — one bit per funnel edge):** `ratingBandVerdict` (`{band, note}` — the rating against the 3.5/4.0/4.5 community-tested conversion bands; null when unrated; the multiplier curve, baselines, and estimated index stay paid-only) and `aiMention` (`{mentioned, model, intent, checkedAt, provenance}` — one canonical intent prompt against one model, 7-day cache, flag-gated by `AI_MENTION_TEASER_ENABLED`, ~$0.0003/probe measured; degrades to absent, never blocks the quote; the multi-prompt multi-model share-of-voice section is paid-only Wave 2).
- **`keywordDiagnosis[]` rows** gain `chance` (1-100, the target app's competitive placement vs the keyword's top-N — null without the target's own score or <3 scored competitors), `kei` (1-100, geometric mean of popularity × chance), and `estMaxDailyImpressions` (`{low, high, source, year}` — SplitMetrics/Phiture 2019 exponential, staleness-labeled). `popularitySource` gains `"observable-signals"`: the documented obs-1 public-signal blend (result depth, leader strength, title-match density, market depth, specificity, exact-phrase) that replaces the unlabeled heuristic whenever the keyword's search results support an estimate; `"apple-search-ads"` remains the labeled overlay when the flag-gated ASA provider returns live data.
- **`metadataMechanics`** (nullable; iOS only): deterministic simulation of documented indexing rules — cross-field duplicates, plural stems, camelCase splits, auto-indexed words, keyword-field format waste — with per-finding `ruleProvenance` (`apple-documented` vs `community-tested`), wasted-character totals, phrase-permutation counts, `keywordsFieldProvided`, and `reviewSafety[]` (App Review / Play-policy risk flags run over the generated ready-to-paste copy). Provenance `inferred`.
- **`conversionAudit`** (nullable): `ratingEconomics` (third-party rating→CVR multiplier curve and per-category baselines shipped as attributed `{low, high, source, year}` ranges, with `ratingBand` verdicts at the 3.5/4.0/4.5 thresholds and the thin-volume caveat), `ratingReset` (the per-territory iOS reset-summary-rating lever: consider/avoid/insufficient-data), and `experimentPlan` (zero-budget PPO / Play-Experiments feasibility: two-proportion sample-size math at 90% confidence / 80% power, MDE 15%; `feasible: null` with a what's-missing recommendation until `ascDailyImpressions` is provided). Provenance `inferred` throughout — these are estimates from public signals plus attributed vendor benchmarks, never measurements.

**Wave 2 additions (2026-06, `reportVersion` ≥ `2026-06-mvp-6`; all additive, both flag-gated server-side):**

- **`aiVisibility`** (nullable; standard/expert tiers; `LLM_PROBE_ENABLED`): LLM share-of-voice probe on the V5-calibrated `v5-10` prompt set — 10 templates × top-2 user keywords × 2 replicates (≤60 calls hard cap, ~$0.02/report measured). Ships `targetSov` with an explicit `sovBand` (`±pp`, basis `v5-pilot-2026-06` — a calibration borrowed from the pilot, not a per-run CI), ranked `shareOfVoice` vs the competitor-trail apps, the per-prompt `promptTable`, and `deterministicMisses` ("never named for 'best free app for X'" — the actionable layer). Mention detection only; no answer text retained. SOV measures one model family's tools-off recommendation recall — store rank is NOT an input. A degraded run (>30% call failures) ships `null`, never partial SOV. Positioning: the per-request/agent-buyable probe (competitors are enterprise demo-gated or subscription) — never claim capability uniqueness.
- **`webDiscoverability`** (nullable; all tiers; `WEB_AUDIT_ENABLED`): deterministic hygiene audit of the listing's marketing domain from four bounded fetches (page HTML ≤512KB/5s, AASA incl. root fallback, assetlinks.json, robots.txt; SSRF-guarded; cached by origin+week with per-app identity re-matching so multi-app domains don't cross-contaminate). Findings: Smart App Banner (+`app-argument`), SoftwareApplication JSON-LD with Google-documented missing required fields, AASA/assetlinks validity + bundleId/package listing, AI-crawler robots directives (GPTBot/PerplexityBot/Google-Extended), OpenGraph presence, and schema-vs-store `ratingDrift`. Hygiene facts only — no web-checkout or install-diversion advice (web-to-app can cannibalize store rank signal). Missing well-known files are findings (`present: false`), not failures; only a failed page fetch nulls the section.
- **`shallowScan.webPlumbing`** (free /quote teaser, §22-safe): three booleans — `smartAppBanner`, `appSchema`, `deepLinking` (AASA on iOS / assetlinks on Android) — from the same weekly cache, raced at 4s so the quote never waits on a slow site (a timeout warms the cache for the next quote). Full audit detail stays paid-only.

**Cost-aware pricing (2026-06, breaking) — the structural fix for x402 pay-first economics:**

The price is locked into the 402 *before* the report runs, so what the orchestrator runs MUST equal what the price paid for. One shared resolver (`payment/cogs.ts` `resolvePaidFeatures` = `(tier defaults ∪ addons) ∩ enabled`) drives BOTH `computePricing` and the orchestrator's COGS gate, so a cheap call can never incur expensive COGS.

- **A missing `tier` now defaults to `standard` ($0.20).** The old $0.03 no-tier base is retired — it ran premium LLM work (AI synthesis + AI-visibility probe + uncapped localization) at a loss. Existing callers that omit `tier` (incl. MCP/CLI, which sent none) now bill standard; the quote/402 shows the price before they sign. Pass `tier: "quick"` for the cheapest deterministic call.
- **`DiagnoseRequest.addons` / `QuoteRequest.addons`** (`{ aiVisibility?, creativeVision?, localizationCopy? }`, all optional): metered premium capabilities, each a `pricing.breakdown[]` line priced so capped COGS ≤ 30% of the line (aiVisibility +$0.10, creativeVision +$0.20, localizationCopy +$0.20). expert bundles all three; standard bundles aiVisibility; quick bundles none. Quote and diagnose MUST be passed identical `addons` (else settle's amount check fails). A server-disabled add-on is neither priced nor run.
- **Tier bundles** (base price carries the bundled COGS): quick = deterministic only; standard = AI synthesis + AI-visibility probe; expert = all four premium features.
- **Refresh discount** applies to base + per-unit lines only, never premium add-on lines.
- **Caps are load-bearing for economics, not just latency:** vision ≤8 downscaled low-detail images on the cheap `VISION_MODEL` (the fix for the $0.18 measurement — uncapped ~25 full-detail images on a full-tier model); localization ≤10 storefronts. These bound the projected COGS the add-on prices are sized against.
- **Margin observability:** a `cogs_ledger` log line per request (revenue, total COGS, margin %, byFeature, overBudget) — the data source for margin alerting. The pack-credit path covers up to expert's projected COGS; premium beyond the bundle requires x402 per-call.
- **Flags** (default off; flip only after a capped re-measure): `VISION_CREATIVE_ENABLED` + `VISION_*` caps, `LOCALIZATION_MAX_STOREFRONTS`. The vision *section* (`creativeAudit` + `serp-creatives` provider + UI) ships next on top of this machinery.
